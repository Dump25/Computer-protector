...
typedef UCHAR state_t[4][4];
static state_t* state;

// The array that stores the round keys.
static UCHAR RoundKey[keyExpSize];
...
// 128bit key
UCHAR key[sizeKey] = {
	(UCHAR)0x2b, (UCHAR)0x7e, (UCHAR)0x15, (UCHAR)0x16,
	(UCHAR)0x28, (UCHAR)0xae, (UCHAR)0xd2, (UCHAR)0xa6,
	(UCHAR)0xab, (UCHAR)0xf7, (UCHAR)0x15, (UCHAR)0x88,
	(UCHAR)0x09, (UCHAR)0xcf, (UCHAR)0x4f, (UCHAR)0x3c };
...
static void AddRoundKey(UCHAR round)
{
	UCHAR i, j;
	for (i = 0; i < 4; ++i)
	{
...
static UCHAR Multiply(UCHAR x, UCHAR y)
{
  return (((y & 1) * x) ^
       ((y>>1 & 1) * xtime(x)) ^
       ((y>>2 & 1) * xtime(xtime(x))) ^
...
static void XorWithIv(UCHAR* buf)
{
	UCHAR i;
...
{
	uintptr_t i;
	UCHAR extra = length % BLOCKLEN; /* Remaining bytes in the last non-full block */
